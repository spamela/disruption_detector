...
threshold=where(bol.data[ttq_window] gt 1.1*a[3])
;result=gaussfit(bol.time[ttq_window], bol.data[ttq_window])

;integrate area under fitted gaussian
tot=int_tabulated(bol.time[ttq_window[threshold]], bol.data[ttq_window[threshold]])
...
oplot, bol.time[ttq_window[threshold]], bol.data[ttq_window[threshold]], color=truecolor('gold')
