...
;some issues with the MWIR - the range runs too long due to noise..
;need to crop this off - smooth the MWIR data when windowing and raise
;threshold from 1.1 stdev to 1.9 stdev (only high frame rate data is
;noisy, low rate data OK as before.

;problems with this if the frame rate is low...return to the old method
;if frame rate is lt 1kHz (arbitary number)
frate=1/(mop.time[1]-mop.time[0])
if(frate gt 1e3) then begin
	;peakstart_m=where(mop.time gt ttq_time-0.004 and smooth(mop.data,10) $
	;			ge thres_mean_m+1.9*thres_stdev_m)
	if max(mop.data) gt 2500 then begin
		peakstart_m=where(mop.time gt ttq_time-0.004 and $
			smooth(mop.data,10) ge thres_mean_m+1.9*thres_stdev_m)
	endif else begin
	;noise to signal bad at low power - bodge away using 23616 as a ref
		print, 'Mod threshold - low power'
		peakstart_m=where(mop.time gt ttq_time-0.004 and $
			smooth(mop.data,15) ge thres_mean_m+2.7*thres_stdev_m)
	endelse
endif else begin
	peakstart_m=where(mop.time gt ttq_time-0.004 and mop.data $
				ge thres_mean_m+1.1*thres_stdev_m)
endelse
...
expansion_mo=max(ratio_mo[exp_window_m])
expansion_mi=max(ratio_mi[exp_window_m])
